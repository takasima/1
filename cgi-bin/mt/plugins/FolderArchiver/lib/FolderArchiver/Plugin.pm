...
sub _build_tmpl {
    ...
    if ( $file ) {
        ...
        $finfo = MT::FileInfo->get_by_key( { templatemap_id => $map->id,
                                             template_id => $tmpl->id,
                                             archive_type => 'Category',
                                             category_id => $category->id,
                                             blog_id => $blog->id, } );
        if ( $finfo->id ) {
            if ( $finfo->file_path && ( $finfo->file_path ne $file ) ) {
                unlink $finfo->file_path;
            }
        }
        $finfo->url( path2url( $file, $blog ) );
        $finfo->file_path( $file );
        # if ( $category ) {
