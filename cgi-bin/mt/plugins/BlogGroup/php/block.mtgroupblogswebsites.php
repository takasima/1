<?php
function smarty_block_mtgroupblogswebsites ( $args, $content, &$ctx, &$repeat ) {
    $args[ 'class' ] = 'blogwebsitegroup';
    $args[ 'child_class' ] = 'blog';
    $args[ 'stash' ] = 'blog';
    $args[ 'search_class' ] = array( 'blog', 'website' );
    $args[ 'prefix' ] = 'blog';
    $this_tag = $ctx->this_tag();
    if (! $this_tag ) return;
    $this_tag = strtolower( $this_tag );
    $this_tag = preg_replace( '/^mt/i', '', $this_tag );
    if ( ( $this_tag != 'groupblogswebsites' )
        && ( $this_tag != 'itemgroupblogs' )
        && ( $this_tag != 'itemgroupblogscount' ) ) {
        $value;
        if (! isset ( $content ) ) {
