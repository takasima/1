<?php
function smarty_block_mtcampaigntags( $args, $content, &$ctx, &$repeat ) {
    ...
    if (! isset( $content ) ) {
        ...
        $blog_id = $campaign->campaign_blog_id;
        $tags = fetch_campaign_tags( $ctx, array( 'campaign_id' => $campaign->campaign_id, 'blog_id' => $blog_id ) );
        if (! is_array( $tags ) ) $tags = array();
        $ctx->stash( '_tags', $tags );
        $ctx->stash( '__out', false );
        $ctx->stash( 'class_type', 'campaign' );
        $counter = 0;
    } else {
...
function fetch_campaign_tags( $ctx, $args ) {
    if (! isset( $args[ 'include_private' ] ) ) {
        $private_filter = 'and (tag_is_private = 0 or tag_is_private is null)';
    }
    if ( isset( $args[ 'campaign_id' ] ) ) {
        if ( isset( $args[ 'tags' ] ) ) {
            if ( isset( $ctx->mt->db()->_campaign_tag_cache[ $args[ 'campaign_id' ] ] ) )
                return $ctx->mt->db()->_campaign_tag_cache[ $args[ 'campaign_id' ] ];
        }
        $campaign_filter = 'and objecttag_object_id = '.intval( $args[ 'campaign_id' ] );
    }
    if ( isset( $args[ 'blog_id' ] ) ) {
        if (! isset( $args[ 'tags' ] ) ) {
            if ( isset( $ctx->mt->db()->_blog_campaign_tag_cache[ $args[ 'blog_id' ] ] ) )
                return $ctx->mt->db()->_blog_campaign_tag_cache[ $args[ 'blog_id' ] ];
        }
    ...
    $sql = "
        ...
        from mt_tag, mt_objecttag, mt_campaign
        where objecttag_tag_id = tag_id
            and campaign_id = objecttag_object_id and objecttag_object_datasource='campaign'
            $blog_filter
            $private_filter
            $tag_filter
            $campaign_filter
    ...
    if ( isset( $args[ 'tags' ] ) ) {
        if ( $args[ 'campaign_id' ] )
            $ctx->mt->db()->_campaign_tag_cache[ $args[ 'campaign_id' ] ] = $tags;
        elseif ( $args[ 'blog_id' ] )
            $ctx->mt->db()->_blog_campaign_tag_cache[ $args[ 'blog_id' ] ] = $tags;
    }
    return $tags;
}
?>