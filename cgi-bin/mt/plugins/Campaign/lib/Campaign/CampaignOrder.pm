package Campaign::CampaignOrder;
use strict;

my $datasource = 'campaignorder';
...
__PACKAGE__->install_properties( {
    ...
    child_of    => [ 'Campaign::Campaign', 'MT::Blog', 'MT::Website' ],
...
sub parents {
    ...
    {   campaign_id => MT->model( 'campaign' ),
        group_id => MT->model( 'campaigngroup' ),
    };
}
...
sub backup_terms_args {
    ...
    if ( defined( $blog_ids ) && scalar( @$blog_ids ) ) {
        return {
            terms   => undef,
            args    => {
                'join' => MT->model( 'campaigngroup' )->join_on(
                    undef,
