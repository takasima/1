...
sub _init_tags {
    ...
    my @fields = MT->model( 'field' )->load( { type => [ 'link', 'link_multi', 'link_group' ] } );
    my $core = MT->component( 'Link' );
...
<span class="field-content" id="field_value-<mt:var name="field_id" escape="html">" style="padding:0px;display:inline;margin-bottom:6px;line-height:1.4em">
...
<MTLinkFieldScope setvar="fieldscope">
...
<span class="actions-bar" style="clear:none;margin-top:4px">
    <span class="actions-bar-inner pkg actions">
        <a href="<mt:var name="script_url">?__mode=list_link&amp;blog_id=<mt:var name="field_blog_id">&amp;dialog_view=1&amp;edit_field=<mt:var name="field_id">&amp;costomobject_select=1" class="mt-open-dialog button">
...
<span class="actions-bar" style="clear:none;margin-top:4px">
    <span class="actions-bar-inner pkg actions">
        <a href="<mt:var name="script_url">?__mode=list_linkgroup&amp;blog_id=<mt:var name="field_blog_id">&amp;dialog_view=1&amp;edit_field=<mt:var name="field_id">&amp;costomobject_select=1" class="mt-open-dialog button">
        <mt:var name="field_label">
        </a>
    </span>
</span>
...
sub _field_html_params {
    ...
    $param->{ field_label } = $plugin->translate( 'Select Link' );
    $param->{ field_class } = $key;
    if ( my $field_id = $param->{ field_value } ) {
        require Link::Link;
        my $obj = Link::Link->load( $field_id );
        if ( $obj ) {
...
sub _field_html_params_multi {
    ...
    if ( my $field_id = $param->{ field_value } ) {
        ...
        my @objects = Link::Link->load( { id => \@ids } );
        my @object_loop;
        my @active_id;
        for my $obj ( @objects ) {
...
sub _field_html_params_group {
    my ( $key, $tmpl, $param ) = @_;
    my $plugin = MT->component( 'Link' );
    $param->{ field_label } = $plugin->translate( 'Select Link Group' );
    $param->{ field_class } = $key;
    if ( my $field_id = $param->{ field_value } ) {
        require Link::LinkGroup;
        my $obj = Link::LinkGroup->load( $field_id );
        if ( $obj ) {
            $param->{ object_exists } = 1;
            my $column_names = $obj->column_names;
            foreach my $column_name ( @$column_names ) {
