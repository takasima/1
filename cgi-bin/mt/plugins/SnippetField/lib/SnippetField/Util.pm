package SnippetField::Util;
use strict;
use lib qw( addons/PowerCMS.pack/lib );
use base qw( Exporter );

our @EXPORT_OK = qw(
    save_asset file_basename move_file uniq_filename file_label
    support_dir upload site_path is_writable is_user_can temporary_asset
);
...
use MT::FileMgr;
use MT::Log;
use MT::Util qw( offset_time_list encode_url );
use PowerCMS::Util qw( is_image utf8_off );
...
sub temporary_asset {
    ...
    my $document_root = quotemeta( $app->document_root );
    my $base = $app->base;
    $url =~ s/^$document_root/$base/;
    $asset = $asset_pkg->new();
    $asset->blog_id( 0 );
    $asset->url( $url );
...
sub save_asset {
    ...
    if ( $cb ) {
        $app->run_callbacks( 'cms_pre_save.asset', $app, $asset, $original )
           or return $app->errtrans( "Saving [_1] failed: [_2]", 'asset',
            $app->errstr );
    }
    $asset->set_tags( @$tags );
    ...
    if ( $obj ) {
        if ( $obj->id ) {
            ...
            my $object_asset = MT::ObjectAsset->get_by_key( { blog_id => ( ( ( ref $obj ) eq 'MT::Website' || ( ref $obj ) eq 'MT::Blog' ) ? $obj->id : $obj->blog_id ),
                                                              asset_id => $asset->id,
                                                              object_id => $obj->id,
...
sub mime_type {
    ...
    my $extension = file_extension( $file );
    my $type = $mime_type{ $extension } || 'text/plain';
    return $type;
}
