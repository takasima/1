...
sub is_valid_extention {
    ...
    if ( $file_extension =~ /(?:docx|xlsx|pptx)$/i ) {
        $suggest = get_ole_extension( $path );
        if ( $file_extension =~ /^$suggest$/i ) {
            return 1;
        }
    } elsif ( File::Extension::Validate->validate( $path ) ) {
        return 1 unless $file_extension =~ /(?:doc|xls|ppt)x?$/i;
        $suggest = get_ole_extension( $path );
        if ( $file_extension =~ /^${suggest}x?$/i ) {
            return 1;
        }
    } elsif ( $file_extension =~ /^txt$/i && -T $path ) {
        return 1;
    } else {
