package CustomObject::CustomObjectOrder;
use strict;
...
__PACKAGE__->install_properties( {
    ...
    child_of    => [ 'CustomObject::CustomObjectGroup', 'MT::Blog', 'MT::Website' ],
...
sub class_label {
    ...
    return $plugin->translate( 'CustomObject Order' );
}
...
sub parents {
    my $obj = shift;
    {   customobject_id => MT->model( 'customobject' ),
        group_id => MT->model( 'customobjectgroup' ),
    };
}
...
sub restore_parent_ids {
    ...
    my ( $data, $objects ) = @_;
    my $result = 0;
    my $customobject_id = $data->{ customobject_id };
    my $group_id = $data->{ group_id };
    my $group_class = $data->{ group_class } || 'customobjectgroup';
    if ( $customobject_id && $group_id && $group_class ) {
...
sub backup_terms_args {
    ...
    if ( defined( $blog_ids ) && scalar( @$blog_ids ) ) {
        return {
            terms => undef,
            args  => {
                'join' => MT->model( 'customobjectgroup' )->join_on(
                    undef,
